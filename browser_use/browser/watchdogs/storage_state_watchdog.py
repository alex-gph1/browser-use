...
class StorageStateWatchdog(BaseWatchdog):
	...
	async def _save_storage_state(self, path: str | None = None) -> None:
		...
		async with self._save_lock:
			# Skip gracefully if CDP root/session is no longer available (e.g. parent timeout/shutdown).
			try:
				cdp_session = await self.browser_session.get_or_create_cdp_session(target_id=None)
			except AssertionError:
				self.logger.debug(
					'[StorageStateWatchdog] Skipping storage state save because CDP root is not initialized '
